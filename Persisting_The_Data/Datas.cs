...
//Console.WriteLine(context.Entry(urun).State ); --> İlk State Detached

//await context.AddAsync(urun); --> Burada göndermiş olduğumuz fonksiyon neticesinde bir state belirleniyor. 

//Console.WriteLine(context.Entry(urun).State); --> Add Fonksiyonundan sonra çalıştırıldığında Added oldu

//await context.SaveChangesAsync();

//Console.WriteLine(context.Entry(urun).State); --> SaveChanges veri tabanına gönderdi ve Unchanged oldu (Eklendikten sonra değişiklik yapılmamış bir veri anlamında)
#endregion
#region Birden Fazla Veri Eklerken Nelere Dikkat Edilmelidir?
#region SaveChanges'ı Verimli Kullanalım!
//SaveChanges fonksiyonu her tetiklendiğinde bir transaction oluşturacağından dolayı EF Core ile yapılan her bir işleme özel kullanmaktan kaçınmalıyız! Çünkü her işleme özel transaction veritaanı açısından ekstradan maliyet demektir. O yüzden mümkün mertebe tüm işlemlerimizi tek bir transaction eşliğinde veritabanına gönderebilmek için savechanges'ı aşağıdaki gibi tek seferde kullanmak hem maliyet hem de yönetilebilirlik açısından katkıda bulunmuş olacaktır.
